...
@PageTitle("Liquidi")
@Route("liquidi")
@Menu(order = 11, icon = LineAwesomeIconUrl.PENCIL_RULER_SOLID)
public class LiquidiView extends Composite<VerticalLayout> {
    ...
    public LiquidiView() {
        ...
        textArea.setLabel("LIQUIDI (indicare quantità) / PRESIDI PRESENTI IN T0");
        ...
        buttonPrimary.addClickListener(event -> {
            buttonPrimary.getUI().ifPresent(ui -> ui.navigate("moulage"));
        });

        buttonPrimary2.addClickListener(event -> {
            buttonPrimary2.getUI().ifPresent(ui -> ui.navigate(""));
        });
    }
}
